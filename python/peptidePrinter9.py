# ==============================================================================
# PROJECT:   PEPTIDE_MARK_I_V87_SNAP_GRID_FIX
# THEME:     Clinical Blue / Medical White
# FOCUS:     25 Clean Placards | Fixed Snap Grid | Full Velocity
# __FILE__:  PeptidePrinter87.py
# __DATE__:  2026-04-03
# __TIME__:  19:45:00
# ==============================================================================
...
class PeptideMarkI(ctk.CTk):
    def __init__(self):
        super().__init__()
        # V77 Project Headers
        print(f"Project: PEPTIDE_MARK_I_V87 | __FILE__: PeptidePrinter87.py")
        print(f"Compile Date: {datetime.now().strftime('%Y-%m-%d')} | Time: {datetime.now().strftime('%H:%M:%S')}")

        self.title("Lyttle reSearch Peptide Crafting Control - NEOANTIGEN EDITION")
        ...
        # --- UI VARIABLES ---
        self.active_profile_name = ctk.StringVar(value="NEO_PEPTIDE_BETA")
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(p_frame, text="HARDWARE PROFILE", text_color="#62B6CB", font=("Arial", 12, "bold")).pack()
        ...
        ctk.CTkLabel(j_frame, text="ACTIVE PEPTIDE SYNTHESIS", text_color="#BEE9E8", font=("Arial", 12, "bold")).pack()
        ...
        # Snap-Alignment (FIXED GRID SPACING)
        ...
        r_grid = ctk.CTkFrame(sn_box, fg_color="transparent"); r_grid.pack(expand=True, fill="both", padx=5)
        ...
        for i, name in enumerate(STATION_NAMES):
            ctk.CTkRadioButton(r_grid, text=name.upper(), variable=self.active_station, value=name, 
                             font=("Arial", 8, "bold"), width=10).grid(row=i//5, column=i%5, sticky="w", padx=2, pady=1)
        ...
        # --- 4. STATION CARDS (CLEAN V77 STYLE) ---
        ...
        for i, name in enumerate(STATION_NAMES):
            ...
            ctk.CTkLabel(card, text=name.upper(), font=("Arial", 11, "bold"), text_color="#1B4965").pack(pady=5)
            ctk.CTkButton(card, text="⚙", width=60, height=45, font=("Arial", 24), 
                        fg_color="transparent", text_color="#1B4965", hover_color="#BEE9E8",
                        command=lambda n=name: self.open_settings(n)).pack(pady=5)
    ...
    def open_settings(self, name):
        ...
        popup = ctk.CTkToplevel(self); popup.title(f"Station Calibration: {name}"); popup.geometry("550x720"); popup.attributes("-topmost", True)
        ...
        ctk.CTkButton(r_f, text="PURGE R", width=85, fg_color="#E63946").pack(side="right")
        ...
        if name in AMINO_ACIDS:
            ...
            ctk.CTkButton(a_f, text="PURGE A", width=85, fg_color="#E63946").pack(side="right")
