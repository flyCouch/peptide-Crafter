# ==============================================================================
# PROJECT:   PEPTIDE_MARK_I_V91_INTEGRATED_JSON_DRO
# THEME:     Clinical Blue / Medical White
# FOCUS:     JSON Export | 1-Decimal DRO | Mantaining ALL V90 Functions
# __FILE__:  PeptidePrinter91.py
# __DATE__:  2026-04-03
# ==============================================================================
...
class StationData:
    ...
    def to_dict(self):
        """Converts station data for JSON export."""
        return {
...
class PeptideMarkI(ctk.CTk):
    def __init__(self):
        super().__init__()
        # Console Header for Serial Debugging
        print(f"Project: PEPTIDE_MARK_I_V91 | __FILE__: PeptidePrinter91.py")
        ...
        # Internal raw values (Floats)
        self.val_x = ctk.DoubleVar(value=0.0)
        self.val_y = ctk.DoubleVar(value=0.0)
        self.val_z = ctk.DoubleVar(value=0.0)
        
        # Display DRO (Formatted Strings)
        self.cur_x = ctk.StringVar(value="0.0")
        self.cur_y = ctk.StringVar(value="0.0")
        self.cur_z = ctk.StringVar(value="0.0")

        # Trace listeners to force 1 decimal place on display
    ...
    def save_config(self):
        """Mechanically saves current state to a JSON file."""
        config_data = {
            "metadata": {
                "profile": self.active_profile_name.get(),
                "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "argon_psi": self.argon_psi.get(),
                "gantry_speeds": {
                    "x": self.speed_x.get(),
                    "y": self.speed_y.get(),
                    "z": self.speed_z.get()
            ...
            "stations": {name: data.to_dict() for name, data in self.storage.items()}
        }
    ...
    def setup_ui(self):
        ...
        # Link Save Button to JSON Function
        ctk.CTkButton(p_btns, text="SAVE", width=105, height=35, fg_color="#62B6CB", text_color="black", 
        ...
        # --- 2. MONITORING BAR (DRO Updated for 1 Decimal) ---
        ...
        # Manual Jog Logic for NEMA 23 Simulation
        ctk.CTkButton(g, text="X-", width=45, command=lambda: self.val_x.set(self.val_x.get()-float(self.jog_step.get()))).grid(row=0, column=0, pady=2)
        ctk.CTkButton(g, text="X+", width=45, command=lambda: self.val_x.set(self.val_x.get()+float(self.jog_step.get()))).grid(row=0, column=1, pady=2)
        ctk.CTkButton(g, text="Y-", width=45, command=lambda: self.val_y.set(self.val_y.get()-float(self.jog_step.get()))).grid(row=1, column=0, pady=2)
        ctk.CTkButton(g, text="Y+", width=45, command=lambda: self.val_y.set(self.val_y.get()+float(self.jog_step.get()))).grid(row=1, column=1, pady=2)
        ctk.CTkButton(g, text="Z-", width=45, command=lambda: self.val_z.set(self.val_z.get()-float(self.jog_step.get()))).grid(row=2, column=0, pady=2)
        ctk.CTkButton(g, text="Z+", width=45, command=lambda: self.val_z.set(self.val_z.get()+float(self.jog_step.get()))).grid(row=2, column=1, pady=2)
