# ==============================================================================
# PROJECT:   PEPTIDE_MARK_I_V114_LIVE_UPDATE
# THEME:     Clinical Blue / Medical White
# FOCUS:     Remove Local Save Button | Enter-to-Update | Profile Persistence
# __FILE__:  PeptidePrinter114.py
# __DATE__:  2026-04-04
# __TIME__:  14:20:00
# ==============================================================================
...
class PeptideMarkI(ctk.CTk):
    def __init__(self):
        super().__init__()
        print(f"Project: PEPTIDE_MARK_I_V114 | __FILE__: PeptidePrinter114.py")
        print(f"Compile Date: {datetime.now().strftime('%Y-%m-%d')} | Time: {datetime.now().strftime('%H:%M:%S')}")

        self.title("Lyttle reSearch Peptide Control - 30 STATION V114")
    ...
    def setup_ui(self):
        ...
        ctk.CTkButton(f_btns, text="LOAD FASTA", fg_color="#5FA8D3", text_color="black", width=140).pack(side="left", padx=5)
        ctk.CTkButton(f_btns, text="GENERATE G-CODE", fg_color="#5FA8D3", text_color="black", width=160).pack(side="left", padx=5)
    ...
    def open_settings(self, name):
        ...
        # We bind directly to master_data so 'Enter' or focus-out applies the value immediately
    ...
    def save_global_profile(self):
        ...
        print(f"GLOBAL PROFILE '{filename}' SAVED SUCCESSFULLY.")
